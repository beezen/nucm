...
import { getConfig, getRegistryConfig } from "../../src/utils/index.js";
describe("action_base", () => {
  ...
  it("addUser", () => {
    ...
    config = getConfig();
    expect(config.nucmrcConfig[registryName][name1]).toBeDefined();
    expect(config.nucmrcConfig[registryName][name2]).toBeDefined();
  });

  it("changeUser", () => {
    expect(config.nucmrcConfig[registryName][name1]["is-current"]).toBeUndefined();
    changeUser(name2);
    config = getConfig();
    expect(config.nucmrcConfig[registryName][name2]["is-current"]).toBeDefined();
  });

  it("removeUser", () => {
    removeUser(name1);
    config = getConfig();
    expect(config.nucmrcConfig[registryName][name1]).toBeUndefined();
  });
